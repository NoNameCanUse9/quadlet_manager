          ...
          <tbody className="divide-y divide-border">
            {networks.map(n => (
              <tr key={n.id} className="hover:bg-surface-raised/50 transition-colors">
                <td className="px-4 py-3 text-text-primary font-semibold font-mono">{n.name}</td>
                <td className="px-4 py-3 text-text-muted font-mono text-xs">{n.id.slice(0, 12)}</td>
      ...
      {deleteTarget && (
        <div className="fixed inset-0 bg-background/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-surface border border-border rounded-lg p-6 max-w-md w-full mx-4 space-y-4">
            ...
            <p className="text-sm text-text-secondary">{t('networks.removeConfirm', { name: deleteTarget })}</p>
            <div className="flex justify-end gap-2 text-sm pt-2">
