    ...
    <div className="space-y-4">
      ...
      <div className="border border-border rounded bg-surface overflow-hidden">
        {loading ? (
          ...
          <div className="p-4 text-xs text-text-muted">{t('networks.noData')}</div>
        ) : (
          <table className="w-full text-xs">
            <thead>
              <tr className="text-text-muted text-left border-b border-border bg-surface-raised">
                <th className="px-3 py-2 font-medium">{t('networks.name')}</th>
                <th className="px-3 py-2 font-medium">{t('networks.id')}</th>
              </tr>
            </thead>
            <tbody>
              {networks.map((n) => (
