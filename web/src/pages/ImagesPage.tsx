        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-surface-raised text-text-secondary border-b border-border">
              <th className="px-4 py-3 text-left font-semibold text-text-muted text-xs uppercase tracking-wider">{t('common.id')}</th>
              <th className="px-4 py-3 text-left font-semibold text-text-muted text-xs uppercase tracking-wider">{t('images.tags')}</th>
              <th className="px-4 py-3 text-right font-semibold text-text-muted text-xs uppercase tracking-wider">{t('images.size')}</th>
          ...
          <tbody className="divide-y divide-border">
            {images.map(img => (
              <tr key={img.id} className="hover:bg-surface-raised/50 transition-colors">
                <td className="px-4 py-3 font-mono text-text-muted">{img.id.slice(0, 12)}</td>
                <td className="px-4 py-3 text-text-primary font-semibold font-mono">{img.tags.join(', ') || '-'}</td>
                <td className="px-4 py-3 text-right text-text-secondary font-mono font-medium">{formatBytes(img.size)}</td>
      ...
      {deleteTarget && (
        <div className="fixed inset-0 bg-background/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-surface border border-border rounded-lg p-6 max-w-md w-full mx-4 space-y-4">
            ...
            <p className="text-sm text-text-secondary">{t('images.removeConfirm')}</p>
            <div className="flex justify-end gap-2 text-sm pt-2">
